...
def make_predictions(args: Namespace, smiles: List[str] = None) -> List[Optional[List[float]]]:
    ...
    # full_data = test_data
    # # test_data = ReactionDataset([test_data[i] for i in valid_indices])
    # test_data = ReactionDataset(full_data)
    ...
    # Predict with each model individually and sum_5_CV predictions
    ...
    for checkpoint_path in tqdm(args.checkpoint_paths, total=len(args.checkpoint_paths)):
        ...
        encoder_output, model_preds = predict(
            model=model,
            data=test_data,
    ...
    avg_ensemble_test_score = np.nanmean(test_scores)
    print(f'Ensemble test {args.metric} = {avg_ensemble_test_score:.6f}')
