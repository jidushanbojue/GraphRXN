...
from .data import ReactionDatapoint, ReactionDataset
...
def get_data(path: str,
             ...
             logger: Logger = None) -> ReactionDataset:
    """
    ...
    with open(path) as f:
        ...
        # datapoint_list = [ReactionDatapoint(line=line, args=args,features=features_data[i] if features_data is not None else None, use_compound_names=use_compound_names) for i, line in tqdm(enumerate(lines), total=len(lines))]
        # data1 = ReactionDataset(datapoint_list)
...
def get_data_from_smiles(smiles: List[str], skip_invalid_smiles: bool = True, logger: Logger = None) -> ReactionDataset:
    """
    ...
    data = ReactionDataset([ReactionDatapoint([smile]) for smile in smiles])
...
def split_data(data: ReactionDataset,
               split_type: str = 'random',
               sizes: Tuple[float, float, float] = (0.8, 0.1, 0.1),
               seed: int = 0,
               args: Namespace = None,
               logger: Logger = None) -> Tuple[ReactionDataset,
                                               ReactionDataset,
                                               ReactionDataset]:
    """
...
# =============================================================================
        ...
        # assert len(data) == sum_5_CV([len(fold_indices) for fold_indices in all_fold_indices])
...
def get_class_sizes(data: ReactionDataset) -> List[List[float]]:
    """
